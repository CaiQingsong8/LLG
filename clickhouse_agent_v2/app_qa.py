...
def get_hierarchy():
    ...
    if not ck:
        return {}, {}
    return fetch_corp_hierarchy(ck)
...
def get_materials():
    ...
    if not ck:
        return {}, [], {}
    return fetch_material_hierarchy(ck)

hierarchy, corp_id_map = get_hierarchy()
mat_hierarchy, all_mats, mat_id_map = get_materials()
...
# 客户联动：根据已选公司获取客户列表
relevant_clients, client_id_map = fetch_customers(get_ck(), st.session_state.selected_corps)
