...
if not API_KEY:
    raise ValueError("❌ 未找到 DS_API_KEY，请在 .env 文件或环境变量中配置")
llm = ChatOpenAI(
...
if __name__ == "__main__":
    test_questions = ["2025年总销量是多少？", "分析下趋势"]
    for q in test_questions:
        print(f"\nUser: {q}")
        for chunk in agent_executor.stream({"messages": [("user", q)]}, stream_mode="values"):
